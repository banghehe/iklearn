...
function get_subdomain()
{
	...
	/*
	if(in_array($subdomain1, array('www')) === true) {
		//return 'www';
		$URL = $_SERVER['REQUEST_URI'];
		$segment = explode('/',$URL);
		if($segment[1] == '' || !isset($segment[1])){ $segment[1] = 'en';}
		header('Location: '.site_url().'/'.$segment[1].'/home');
		die();
	}
	*/else{
		$URL = $_SERVER['REQUEST_URI'];
		...
		if($segment[1] == ''){
			...
			header('Location: '.site_url().'/'.$segment[1].'/home');
			die();
		}elseif($fulldomain[0] == 'www' && empty($segment[2])){
			header('Location: '.site_url().'/'.$segment[1].'/home');
			die();
		}
	}
...
function dictionary_init() {
    if(!session_id()) {
		...
		ini_set( 'session.cookie_domain', str_replace('https://', '',site_url()) );
        session_start();
    }
