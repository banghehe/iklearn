 ...
 * home url with language code
 */
function local_home_url($path = '')
{
	$path = ltrim($path, '/');
	$locale_parts = explode('_', get_locale());
	$locale_code = empty($path) ? $locale_parts[0] : $locale_parts[0] . '/';
	return home_url($locale_code . $path.'/home');
}
...
function site_math_url_not_langue()
{
	$url = site_url();
...
function get_sat_class_score($group_id)
{
	...
	$query = 'SELECT h.id as hid, h.sheet_id as sid, correct_answers_count ,sheet_name , score, submitted_on, for_practice, h.created_by
...
{    
	...
	$ignored = array(
		'home', 'login', 'signup', 'ajax', 'api','ensat', // english site
		...
		'spelling_practice', 'vocabulary_practice', 'reading_comprehension', 'writing_practice', 'sat_preparation', 'sat_preparation/writing', 'sat_preparation/vocab', 'sat_preparation/sat1', 'sat_preparation/sat2', 'sat_preparation/sat3', 'sat_preparation/sat4', 'sat_preparation/sat5', // english site
		'teaching', 'teaching/register', 'about_us', 'manage_subscription', 'my_account', 'online_learning',// english site

		'calculus', 'geometry', 'algebra_ii', 'algebra_i', 'arithmetics', 'arithmetics', 'math_homework', 'sat_preparation/sat1prep', 'sat_preparation/sat1a', 'sat_preparation/sat1b', 'sat_preparation/sat1c', 'sat_preparation/sat1d', 'sat_preparation/sat1e' , 'sat_preparation/sat2prep', 'sat_preparation/sat2a', 'sat_preparation/sat2b', 'sat_preparation/sat2c', 'sat_preparation/sat2d', 'sat_preparation/sat2e','sat_preparation/emathk' // math site
	);
...
{          
	...
	$obj->count = $total;//count($_SESSION['search_history'][$dictionary]);
 ...
 * @param $title
 */
function ik_enqueue_messages($msg, $type = 'message', $title = 'Messages', $other = array(),$color='',$msg_second='' )
{
...
/*
 * Return current toast queue, empty the queue after that.
 * Return false if the queue is empty
 *
 * @return mixed
 */
//function ik_get_toast_queue()
//{
//	$current_user_id = get_current_user_id();
//
//	$messages = get_user_meta($current_user_id, 'ik-toast', true);
//
//	if(empty($messages)) {
//		$messages = empty($_SESSION['toast']) ? false : $_SESSION['toast'];
//	}
//	else {
//		update_user_meta($current_user_id, 'ik-toast', array());
//	}
//
//	$_SESSION['toast_queue'] = array();
//
//	return $messages;
//}
/*
...
function check_admin_by_id($userid){
	$user =  get_user_by( 'id', $userid );
	if(in_array( 'administrator', $user->roles )) 	return true;
	elseif(in_array( 'mw_super_admin', $user->roles )) 	return true;
	else return false;
}
