...
function ik_is_teaching_agreement_agreed($type = '')
{
	if(empty($type)) {
		...
		$date = get_user_meta(get_current_user_id(), 'math_teacher_agreement_ver', true);
	}
	return !empty($date);
}
...
function ik_agree_teaching_agreement($type = '')
{
	...
	if(empty($type)) {
		...
		update_user_meta(get_current_user_id(), 'math_teacher_agreement_ver', $agreement_update_date);
	}
}
 ...
 * @return boolean
 */
function ik_request_worksheet_grading($homework_result_id, $homework_id, $user_id = 0,$status=0)
{
	$user_id = $user_id ? $user_id : get_current_user_id();
	...
	$user_points = ik_get_user_points($user_id);
        ...
        if($status==1){
            $homework = MWDB::get_id_homework_result_sheets($homework_id)->price;
        }else{
...
/**
 * pay group price if join group
 */
function ik_join_group($price, $user_id = 0)
{
	$user_id = $user_id ? $user_id : get_current_user_id();

	$user_points = ik_get_user_points($user_id);
	if($user_points < $price) {
		ik_enqueue_messages(__('You don\'t have enough points to join group.', 'iii-dictionary'), 'error');
		return false;
	}else{
		ik_deduct_user_points($price, $user_id);
		return true;
	}
 ...
 * @return boolean
 */
function ik_finish_grading_request($request_id)
{
	global $wpdb;
