<?php
	...
	$actual_link_current = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        $actual_link = strstr($actual_link_current, 'hid');
//        var_dump($actual_link);die;
	$is_math_panel = is_math_panel();
	$_page_title = __('Homework Result', 'iii-dictionary');
        if(isset($hrid)){
            
	$homework_result = MWDB::get_homework_results_not_group($hrid, $sid,$grid);
        }else{
	...
	<div class="row">
		...
		<div class="col-sm-3">
                    ...
                    if($actual_link==false){ ?>
                        <a href="<?php echo locale_home_url() . '/?r=online-learning' ?>" class="btn btn-default btn-block grey form-control"><span class="icon-goback"></span><?php _e('Back', 'iii-dictionary') ?></a>
		    <?php }else{ ?>
