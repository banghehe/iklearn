...
//    var_dump($_POST);die;
    ...
    if (!empty($_SESSION['return_math'])) {
        ...
        wp_redirect(home_url_ssl() . '/?r=my-account');
    }
    exit;
}
...
<script>
    (function ($) {
        jQuery('#payments .article-header').css('background', '#ffffff');
        jQuery('#payments #page-tabs-container').css('background', '#ffffff');
        jQuery('#payments .entry-content').css('background', '#ffffff');
        jQuery('#payments .entry-content').css('color', '#000000');
        ...
        jQuery('#payments .article-header .row').attr('style', 'width:1050px; margin:auto !important');
        jQuery('#payments .entry-content .row:first').attr('style', 'width:1050px; margin:auto !important');
    }
    })(jQuery);
</script>
                                    ...
                                    if (!empty($cart_items)) :
                                        ...
                                        foreach ($cart_items as $key => $item) :
                                            ...
                                            <tr>
                                                ...
                                                ?></td>
                                                <?php
                                                    array_push($arr_date,$item->no_months);
                                                ?>
...
<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top" class="hidden">
    ...
    <input type="hidden" name="return" value="<?php echo home_url_ssl() ?>/?r=payments">
        ...
        // Handing add data on table wp_dict_tutoring_plan only pay with "tutoring plan"
            $('#process-btn').click(function(e){
                ...
                if($('#type-payment').val()==1){
                    ...
                    var arr_date_tutoring = [];
                    var array= <?php echo json_encode($arr_date); ?>;
                    var point = $('#total-amount').html();
                    for(var i = 0; i<array.length; i++) {
                        if (array[i].indexOf('-')) {
                            var string = array[i];
                            var string1 = string.split('/');
                            var stt = string1[2]+'-'+string1[0]+'-'+string1[1];
                            arr_date_tutoring.push(stt);
                        }
                    }
                    $.get(home_url + "/?r=ajax/update_wp_dict_tutoring", {arr:arr_date_tutoring,point:point}, function (data) {
                    }); 
                }
            });
