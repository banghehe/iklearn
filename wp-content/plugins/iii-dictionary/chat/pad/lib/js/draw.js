...
$(function () {
    ...
    $('#btn_clear').click(function () {
        ...
        $(document).find('canvas').not("canvas[id*=new-layer-q]").css({'display': 'block'});
...
//        App.socket.emit('click_step', {'id_step': $(this).attr("data-n"), 'client': client, 'idroom': idroom, 'idteacher': idteacher, 'id_click': 0});
    });
//    App.socket.on('update_click_step', function (data) {
//        if (data.idteacher == client && data.idroom == idroom && data.client == idteacher) {
...
$(function () {
    App.socket.on('connect', function () {
        ...
        $('#btn_import').on('pointerdown', function () {
            jQuery('#btn_import').css('background','#a3b1f5');
        ...
        $('#lbl_image').on('click', function () {
            jQuery('#lbl_image').css('background','#a3b1f5');
        });
        ...
        App.socket.on('update_canvas', function (data) {
            if (data.canvas.uid == idteacher && data.canvas.iduser == client && data.canvas.idroom == idroom) {
...
function canDraw(_canvas) {
    ...
    $(_canvas)
            ...
            .mouseup(function (e) {
                dataURL(client, idteacher, idroom, sid, $('#nlayer_step li.use_layer').attr('data-n'), $('#nlayer_step li.use_layer').text(), $('canvas.can-paint'));
                isDown = false;
                _ctx.closePath();
