...
jQuery('#open-notepad-btn').on('click',function() {
	//send data to socket.io
        get_all_image();
	get_all_nav();
        if(jQuery('#sl-reciplent').attr('id-teacher')==undefined){
	...
	jQuery( "#auto-open-chat" ).attr('href','/wp-content/plugins/iii-dictionary/chat/pad/Draw.html?client=' 
                ...
                +__PRICE+'&CURRENTNAME='+CURRENTNAME +'&IDSTUDENT='+ jQuery('#sl-reciplent').attr('id-teacher') 
                + '&IDROOM='+jQuery('#sl-reciplent').attr('IDROOM')+'&EMAILTEACHER='+jQuery('#sl-reciplent').attr('data-email'));
