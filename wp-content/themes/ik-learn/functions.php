...
require_once( 'library/custom-ajax-auth.php' );
...
// remove auto insert <p>, <br> fillter
    ...
    register_nav_menus(array(
        ...
        'menu-sat-math-english' => 'Menu Sat Math English',
        'ensat-nav-english' => 'Ensat English Menu',
    ));
    ...
    function prefix_ajax_add_foobar() {
        if (isset($_REQUEST['gname']) && isset($_REQUEST['gpass'])) {
            $gname=$_REQUEST['gname'];
            $gpass=$_REQUEST['gpass'];
            $data = MWDB::get_group_price($gname, $gpass)->price;
            if(is_null($data)){
                $data=0;
            }
...
function chat_styles() {
   ...
   */
  wp_register_style( 'chat-style', get_template_directory_uri() . '/style.css', 'all' );
  wp_enqueue_style( 'chat-style' );
}
...
function add_current_nav_class($classes, $item) {
    ...
    if(isset($route[1])) {
            switch($route[1]) {
                    case 'elearner': $id = 68;
                            break;
                    case 'collegiate': $id = 67;
                            break;
                    case 'medical': $id = 66;
                            break;
                    case 'intermediate': $id = 65;
                            break;
                    case 'elementary': $id = 64;
                            break;
            }
    }
