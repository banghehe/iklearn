var ASSIGNMENT_SPELLING = 1;
var ASSIGNMENT_VOCAB_GRAMMAR = 2;
var ASSIGNMENT_READING = 3;
var ASSIGNMENT_WRITING = 4;
var ASSIGNMENT_VOCAB_BUILDER = 5;
var ASSIGNMENT_REPORT = 6;
(function ($) {
    $(function () {
        $("#btn-main-menu").click(function () {
            $("#main-nav").slideToggle().toggleClass("show")
        });
        $("#btn-sub-menu").click(function () {
            $("#sub-user-nav").slideToggle()
            $("#lang-switcher-nav").slideToggle()
        });
        $('#btn-main-menu').click(function(){
//            if ($(window).width() < 768) {
//                jQuery('div > nav:first-of-type').hide();
//            }else{
                jQuery('div > nav:first-of-type').show();
//            }
        });
        if (window.location.href.indexOf('home') > -1 && window.location.href.indexOf('math')==-1) {
            $("#sub-user-nav").find("> ul > li:nth-child(2) > a").click(function (e) {
                var viewport = getViewport();
                if (viewport.width <= 1199) {
                    e.preventDefault();
                    $("#sub-user-nav").find("> ul > li > .sub-menu").slideUp();
                    $("#lang-switcher-nav").find("> ul > li > .sub-menu").slideUp();
                    var menu = $(this).next();
...
//            $("#lang-switcher-nav").find("> ul > li > .sub-menu").css("display", "");
            $("#lang-switcher-nav").css("display", "");
            ...
            if(viewport.width < 925){
                if($(".main-my-schedule").hasClass('active-tab-schedule')){
                    ...
                    if(viewport.width < 650){
                        $('#tab-tutor-content .border-selectall').find('.col-md-6').css('width','53.5%');
                    }else{
        ...
        $(".scroll-list-1").mCustomScrollbar({
            theme: "rounded-dark",
            mouseWheel: {scrollAmount: 120},
            callbacks: {
                onOverflowY: function () {
        ...
        $('.go-now').click(function(e){
            jQuery($(this)).next().css('display','block');
            jQuery('.go-now').not($(this)).next().css('display','none');
        });
        
        $("body").popover({selector: "[data-toggle='popover']", trigger: '"' + $(this).attr("data-trigger") + '"'})
                .on("show.bs.popover", function () {
                    $(this).data("bs.popover").tip().css("max-width", $(this).attr("data-max-width"))
                });
...
function setup_popover(selector, title, readonly, textarea, max_width) {
    ...
    jQuery(selector).popover({
        ...
        "show.bs.popover": function () {
            jQuery(this).data("bs.popover").tip().css("max-width", max_width);
        },
...
function setup_homework_viewer(assignment) {
    ...
    if (assignment == "4") {
        ...
        jQuery("#questions-table tr").each(function (index) {
            index++;
            spelling_html += "<li> No." + index + " - " + jQuery(this).find("td:nth-child(1) input").val() + "</li>";
        })
        spelling_html += "</ul>";
